  ...
  <div>
    ...
    <div v-else-if="player.state === 'READING' || !player.state && stories.length">
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
        <div style="font-weight: bold; color: #aaa; font-size: 0.88em; letter-spacing: 0.5px;">STORYS</div>
        <div class="view-toggle">
          <button :class="['view-toggle-btn', {active: viewMode === 'contributions'}]" @click="viewMode = 'contributions'">Beiträge</button>
          <button :class="['view-toggle-btn', {active: viewMode === 'prose'}]" @click="viewMode = 'prose'">Fließtext</button>
        </div>
      </div>
      ...
      <div style="text-align: left">
        <div v-for="(story, i) in stories" :key="i">
          ...
          <!-- Beiträge-Ansicht -->
          <sui-card v-if="viewMode === 'contributions'">
            <div class="like-bar">
          ...
          <sui-card v-else>
            ...
            <sui-card-content style="margin-bottom: 14px;">
              <p style="font-family: 'Lora', serif; line-height: 1.75; white-space: pre-wrap;">{{ story.map(e => e.link).join(' ') }}</p>
            </sui-card-content>
          </sui-card>
        </div>
      </div>
